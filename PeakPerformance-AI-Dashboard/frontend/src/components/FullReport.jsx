...
function Hexagon({ scores, overall }) {
  ...
  const labels = { Speed: [cx + R + 24, cy], Agility: pt(1, 1.35), Power: pt(2, 1.35), Endurance: [cx - R - 40, cy], Reaction: pt(4, 1.35), Balance: pt(5, 1.35) }
  return (
    <svg viewBox={`0 0 ${size} ${size}`} style={{ width: '100%', maxWidth: 340 }}>
      ...
      {order.map(n => { const [x, y] = labels[n]; return <text key={n} x={x} y={y} textAnchor="middle" fontFamily="Inter" fontSize="12" fill={MUTED}>{n}</text> })}
    </svg>
  )
}
